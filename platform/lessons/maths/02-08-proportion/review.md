---
title: Review & practice
checks:
  - q: '$y$ is directly proportional to $x$. When $x = 5$, $y = 35$. Find $k$.'
    options: ["$k = 7$", "$k = 30$", "$k = 175$", "$k = \\dfrac{1}{7}$"]
    correct: 0
    explain: '$y = kx$, so $35 = 5k$ giving $k = 7$.'
  - q: '$y \propto x^2$. When $x = 4$, $y = 48$. Find $y$ when $x = 6$.'
    options: ["$108$", "$72$", "$144$", "$96$"]
    correct: 0
    explain: '$y = kx^2$. $48 = 16k$ → $k = 3$. At $x = 6$: $y = 3 \times 36 = 108$.'
  - q: '$y$ is inversely proportional to $x$. When $x = 6$, $y = 5$. Find $y$ when $x = 15$.'
    options: ["$y = 2$", "$y = 25$", "$y = \\dfrac{2}{3}$", "$y = 30$"]
    correct: 0
    explain: '$y = \dfrac{k}{x}$. $5 = \dfrac{k}{6}$ → $k = 30$. At $x = 15$: $y = \dfrac{30}{15} = 2$.'
  - q: '$y \propto \dfrac{1}{x^2}$. When $x = 3$, $y = 4$. Find $y$ when $x = 6$.'
    options: ["$y = 1$", "$y = 2$", "$y = 8$", "$y = 16$"]
    correct: 0
    explain: '$y = \dfrac{k}{x^2}$. $4 = \dfrac{k}{9}$ → $k = 36$. At $x = 6$: $y = \dfrac{36}{36} = 1$.'
  - q: 'For $y \propto x^2$, when $x$ triples, $y$ becomes...'
    options: ["$3y$", "$9y$", "$\\dfrac{y}{3}$", "$\\dfrac{y}{9}$"]
    correct: 1
    explain: '$y = kx^2$. New $x = 3x$, new $y = k(3x)^2 = 9 k x^2 = 9y$.'
  - q: 'For $y \propto \dfrac{1}{x}$, when $x$ doubles, $y$ becomes...'
    options: ["$2y$", "$4y$", "$\\dfrac{y}{2}$", "$\\dfrac{y}{4}$"]
    correct: 2
    explain: '$y = \dfrac{k}{x}$. New $x = 2x$, new $y = \dfrac{k}{2x} = \dfrac{y}{2}$. Halves.'
  - q: 'A graph of $y$ vs $x$ shows a straight line through the origin. The relationship is...'
    options: ["direct proportion", "inverse proportion", "neither", "both"]
    correct: 0
    explain: 'A straight line through the origin (with non-zero slope) is the signature of $y = kx$, i.e. direct proportion.'
  - q: 'The volume $V$ of a sphere is directly proportional to the cube of its radius $r$. If the radius doubles, the volume...'
    options: ["doubles", "quadruples", "is multiplied by $6$", "is multiplied by $8$"]
    correct: 3
    explain: '$V \propto r^3$. Doubling $r$ multiplies $V$ by $2^3 = 8$.'
---

## What you should know by now

If you've worked through P1 to P3 you can:

- Translate "$y$ is proportional to $x$" (and its variants) into the right algebraic equation.
- Find the **constant of proportionality** $k$ from one given pair.
- Use the equation to find any other unknown.
- Handle direct, inverse, and powered/root variants ($x^2$, $x^3$, $\sqrt{x}$, $\dfrac{1}{x^2}$).
- Predict how $y$ scales when $x$ doubles (or any factor) for any of the forms.
- Recognise direct proportion's straight-line-through-origin graph and inverse proportion's hyperbola.

## Marking patterns at a glance

This sub-topic is heavily tested on Paper 2:

- **B1** for "find the constant of proportionality" given a direct equation.
- **M1 + A1** for "find $y$ when $x = N$" (set up + answer).
- **M1, M1, A1** for a multi-part word problem (write the equation, find $k$, use it).
- **B1** for "what happens to $y$ when $x$ doubles" : apply the power.

The trap markers Cambridge uses repeatedly:

- "Inversely proportional to $x^2$" → $\dfrac{k}{x^2}$, not $\dfrac{1}{kx^2}$.
- "Proportional to the square of $x$" → $kx^2$, not $\sqrt{kx}$.
- Graphs: a straight line that does NOT pass through the origin is NOT direct proportion ($y = mx + c$ with $c \ne 0$).

## Worked example: a 4-mark Cambridge style

The brightness $B$ of a star is inversely proportional to the square of its distance $d$ from Earth.

A star at distance $d = 5$ light-years has brightness $B = 36$ units.

(a) Find the constant of proportionality.

(b) Find $B$ at $d = 12$ light-years.

(c) At what distance is $B = 4$ units?

**Step 1.** $B = \dfrac{k}{d^2}$.

**Step 2 (a).** $36 = \dfrac{k}{25}$, so $k = 900$.

**Step 3 (b).** $B = \dfrac{900}{144} = 6.25$ units.

**Step 4 (c).** $4 = \dfrac{900}{d^2}$, so $d^2 = 225$ and $d = 15$ light-years.

**Answer:** **(a)** $k = 900$. **(b)** $B = 6.25$. **(c)** $d = 15$. **[M1 for setup; A1 for $k$; A1 for (b); M1 + A1 for (c)]**

Full marks $= 5$.

## Quick reference

- **Direct**: $y = kx^n$. Graph through origin.
- **Inverse**: $y = \dfrac{k}{x^n}$. Hyperbola, both axes as asymptotes.
- **The recipe**: write equation → find $k$ → use it.
- **Scaling**: doubling $x$ multiplies $y$ by $2^n$ (direct) or divides by $2^n$ (inverse).

## Practice: pick the proportion type and find $k$

The faded yellow curve is the target. First pick the relationship type (direct, inverse, square, square-root). That fixes the SHAPE. Then slide $k$ until your blue curve covers the yellow one.

<iframe src="/widgets/match-trainer.html?mode=proportion" style="width:100%;max-width:696px;height:560px;border:0;border-radius:10px;display:block;margin:18px auto" loading="lazy"></iframe>

## Your turn: full quiz

The widget below is a mixed quiz drawing from all three parts.
